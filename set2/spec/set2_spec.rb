...
describe "Set2" do
  describe ".find_intersection" do
    xit "will return empty array for 3 empty arrays" do
      expect(Set2.find_intersection([],[],[])).to eq([])
    end

    xit "will return empty array for 3 empty arrays with no common elements" do
    ...
    xit "will return an array with 1 element if that's the only common one" do
      expect(Set2.find_intersection([7],[7],[7])).to eq([7])
      expect(Set2.find_intersection([15, 20, 7, 5, 10],[12, 7, 24, 18, 6],[7, 14, 28, 21])).to eq([7])
    end

    xit "will return an array with all intersecting values" do
    ...
    xit "will not include values that are only present in 2 arrays" do
  ...
  describe ".find_first_dup" do
    xit "will return nil if there are no duplicates" do
      expect(Set2.find_first_dup([1, 2, 3, 4, 5])).to eq(nil)
    end

    xit "will return the first duplicate in the array" do
  ...
  describe ".find_mode" do
    xit "will return nil if there are no elements" do
      expect(Set2.find_mode([])).to eq(nil)
    end

    xit "will return the mode of the array" do
